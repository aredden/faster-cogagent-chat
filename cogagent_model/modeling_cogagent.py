...
def attention_fn(
    ...
    *,
    ...
    is_full = (attention_mask == 0).all()

    if scaling_attention_score and from_ == "vision_expert":
        dropout_p = (
            0.0
        ...
        return torch.nn.functional.scaled_dot_product_attention(
            ...
            key_layer,
            value_layer,
            causal_lower_right(value_layer.shape[-2], query_layer.shape[-2]),
            0,
        )
    else:
...
class VisionExpertAttention(nn.Module):
    ...
    def forward(
        ...
        novision = vision_token_mask.sum() == 0
        nolanguage = language_token_mask.sum() == 0
        if novision:
...
class CrossAttention(nn.Module):
    ...
    def forward(
        ...
        context_layer = context_layer.transpose(1, 2).reshape(
            bsz, q_len, self.cross_hidden_size
        )
...
class CogAgentDecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.post_cross_attention_layernorm = FusedRMSNorm(
            config.hidden_size, eps=config.rms_norm_eps, elementwise_affine=True
        )
    ...
    def forward(
        ...
        (
            attention_output,
            self_cross_attn_weights,
            present_cross_key_value,
        ) = self.cross_attn(
    ...
    def build_conversation_input_ids(
        ...
        *,
        ...
        if images is not None and len(images) == 1:
            ...
            images = [transform(ori[0]).to(self.device)]
            ...
            cross_images = [cross_transform(ori[0]).to(self.device)]
            # language
